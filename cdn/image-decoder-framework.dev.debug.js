...
new DependencyWorkersTaskHandle(internalContext,callbacks);if(addResult.isNew){internalContext.setParentList(this._internalContexts,addResult.iterator);this._startNewTask(taskKey,internalContext)}return taskHandle};DependencyWorkers.prototype._startNewTask=function startNewTask(taskKey,internalContext){taskContext=this._workerInputRetreiver["createTaskContext"](taskKey,{"onDataReadyToProcess":onDataReadyToProcess,"onTerminated":internalContext.onTerminatedBound});internalContext.taskContext=taskContext;
var dependsOnTasks=taskContext["dependsOnTasks"];if(!taskContext["statusUpdated"])throw"AsyncProxy.DependencyWorkers: missing "+"taskContext.statusUpdated()";if(dependsOnTasks&&!taskContext["onDependencyTaskResult"])throw"AsyncProxy.DependencyWorkers: Cannot accept "+"dependsOnTasks without onDependencyTaskResult in "+"taskContext";var that=this;internalContext.gotDataFromDependsTaskHandles=new Array(dependsOnTasks.length);for(var i=0;i<dependsOnTasks.length;++i){internalContext.gotDataFromDependsTaskHandles[i]=
false;var dependencyTaskHandle;(function closure(index){var isTerminated=false;dependencyTaskHandle=that.startTask(dependsOnTasks[index],{"onData":onDependencyTaskResult,"onTerminated":onDependencyTaskTerminated});function onDependencyTaskResult(data){internalContext.taskContext["onDependencyTaskResult"](data,dependsOnTasks[index]);internalContext.gotDataFromDependsTaskHandles[index]=true}function onDependencyTaskTerminated(){if(isTerminated)throw"AsyncProxy.DependencyWorkers: Double termination";
isTerminated=true;internalContext.dependsTaskTerminated()}})(i);internalContext.dependsTaskHandles.push(dependencyTaskHandle)}setTimeout(function(){for(var i=0;i<internalContext.dependsTaskHandles.length;++i){var handle=internalContext.dependsTaskHandles[i];if(!internalContext.gotDataFromDependsTaskHandles[i]&&handle.hasData())taskContext["onDependencyTaskResult"](handle.getLastData(),dependsOnTasks[i])}});function onDataReadyToProcess(newDataToProcess){if(internalContext.isTerminated)throw"AsyncProxy.DependencyWorkers: already terminated";
else if(internalContext.isActiveWorker){internalContext.pendingDataForWorker=newDataToProcess;internalContext.isPendingDataForWorker=true}else that._startWorker(internalContext,newDataToProcess,taskKey)}};DependencyWorkers.prototype._startWorker=function startWorker(internalContext,dataToProcess,taskKey){var that=this;var worker;if(that._workerPool.length>0)worker=that._workerPool.pop();else worker=new AsyncProxyMaster(that._scriptsToImport,that._ctorName,that._ctorArgs);internalContext.isActiveWorker=
true;internalContext.statusUpdate();worker.callFunction("start",[dataToProcess,taskKey],{"isReturnPromise":true}).then(function(processedData){internalContext.hasProcessedData=true;internalContext.lastProcessedData=processedData;var handles=internalContext.taskHandles;var iterator=handles.getFirstIterator();while(iterator!=null){var handle=handles.getValue(iterator);iterator=handles.getNextIterator(iterator);handle._callbacks["onData"](processedData,taskKey)}return processedData})["catch"](function(e){console.log("Error in DependencyWorkers' worker: "+
e);return e}).then(function(result){that._workerPool.push(worker);if(!internalContext.isPendingDataForWorker){internalContext.isActiveWorker=false;internalContext.statusUpdate();return}var dataToProcess=internalContext.pendingDataForWorker;internalContext.isPendingDataForWorker=false;internalContext.pendingDataForWorker=null;that._startWorker(internalContext,dataToProcess);return result})};asyncProxyScriptBlob.addMember(DependencyWorkersClosure,"DependencyWorkers");return DependencyWorkers}
var DependencyWorkers=DependencyWorkersClosure();var DependencyWorkersTaskHandle=function DependencyWorkersTaskHandleClosure(){function DependencyWorkersTaskHandle(internalContext,callbacks){this._internalContext=internalContext;this._localPriority=0;this._callbacks=callbacks;this._taskHandlesIterator=internalContext.taskHandles.add(this)}DependencyWorkersTaskHandle.prototype.hasData=function hasData(){return this._internalContext.hasProcessedData};DependencyWorkersTaskHandle.prototype.getLastData=function getLastData(){return this._internalContext.lastProcessedData};
...
asyncProxyScriptBlob.addMember(DependencyWorkersTaskHandleClosure,"DependencyWorkersTaskHandle");return DependencyWorkersTaskHandle}();var DependencyWorkersInternalContext=function DependencyWorkersInternalContextClosure(){function DependencyWorkersInternalContext(){this.isTerminated=false;this.priority=0;this.lastProcessedData=null;this.hasProcessedData=false;this.isActiveWorker=false;this.isPendingDataForWorker=false;this.pendingDataForWorker=null;this.taskContext=null;this.dependsTaskHandles=[];this.taskHandles=new LinkedList;this.gotDataFromDependsTaskHandles=[];this.dependsTasksTerminatedCount=0;this.onTerminatedBound=this.onTerminated.bind(this)}
DependencyWorkersInternalContext.prototype.setParentList=function(list,iterator){this._parentList=list;this._parentIterator=iterator};DependencyWorkersInternalContext.prototype.ended=function(){var dependsHandles=this.dependsTaskHandles;for(var i=0;i<dependsHandles.length;++i)dependsHandles[i].unregister();var iterator=this.taskHandles.getFirstIterator();while(iterator!=null){var handle=this.taskHandles.getValue(iterator);iterator=this.taskHandles.getNextIterator(iterator);if(handle._callbacks["onTerminated"])handle._callbacks["onTerminated"]()}this.taskHandles.clear();
this.dependsTaskHandles=[];this._parentList.remove(this._parentIterator);this._parentIterator=null};DependencyWorkersInternalContext.prototype.setPriorityAndNotify=function(newPriority){if(this.priority===newPriority)return;this.priority=newPriority;this.statusUpdate();var depends=this.dependsTaskHandles;for(var i=0;i<depends.length;++i)depends[i]["setPriority"](newPriority)};DependencyWorkersInternalContext.prototype.statusUpdate=function(){this.taskContext["statusUpdated"]({priority:this.priority,
hasListeners:this.taskHandles.getCount()>0,isIdle:!this.isActiveWorker,terminatedDependsTasks:this.dependsTasksTerminatedCount})};DependencyWorkersInternalContext.prototype.recalculatePriority=function(){var handles=this.taskHandles;var iterator=handles.getFirstIterator();var isFirst=true;var newPriority=0;while(iterator!=null){var handle=handles.getValue(iterator);if(isFirst||handle._localPriority>newPriority)newPriority=handle._localPriority;iterator=handles.getNextIterator(iterator)}return newPriority};
DependencyWorkersInternalContext.prototype.onTerminated=function(){if(this.isTerminated)throw"AsyncProxy.DependencyWorkers: already terminated";else if(this.isActiveWorker)throw"AsyncProxy.DependencyWorkers: Cannot terminate while "+"task is processing. Wait for statusUpdated() callback "+"with isIdle == true";this.isTerminated=true;this.ended()};DependencyWorkersInternalContext.prototype.dependsTaskTerminated=function(){++this.dependsTasksTerminatedCount;this.statusUpdate()};return DependencyWorkersInternalContext}();function PromiseTaskClosure(){var asyncProxyScriptBlob=self["asyncProxyScriptBlob"];var WAITING_FOR_DEPENDS_TASKS=1;var WAITING_FOR_PROMISE=2;var WAITING_FOR_WORKER=3;var TERMINATED=4;function PromiseTask(taskKey,dependsOnTasks,workerInputRetreiver,callbacks){var that=this;that._taskKey=taskKey;that._dependsOnTasks=dependsOnTasks;that._workerInputRetreiver=workerInputRetreiver;that._callbacks=callbacks;that._resultIndexByTaskKey=null;that._results=[];that._hasResultByIndex=[];that._hasResultCount=
0;that._waitingFor=WAITING_FOR_DEPENDS_TASKS;that._checkIfDependsTaskDone()}Object.defineProperty(PromiseTask.prototype,"dependsOnTasks",{get:function(){var that=this;return that._dependsOnTasks}});PromiseTask.prototype.onDependencyTaskResult=function(result,key){if(this._waitingFor!==WAITING_FOR_DEPENDS_TASKS)throw"AsyncProxy.PromiseTask: Internal Error: not waiting for "+"tasks depends on";if(this._resultIndexByTaskKey===null){this._resultIndexByTaskKey=new HashMap(this._workerInputRetreiver);for(var i=
0;i<this._dependsOnTasks.length;++i)this._resultIndexByTaskKey.tryAdd(this._dependsOnTasks[i],function(){return i})}var index=this._resultIndexByTaskKey.getFromKey(key);if(index===null)throw"AsyncProxy.PromiseTask: Task is not depends on key";this._results[index]=result;if(!this._hasResultByIndex[index]){this._hasResultByIndex[index]=true;++this._hasResultCount}};PromiseTask.prototype.statusUpdated=function(status){if(!status.hasListeners&&status.isIdle)this._terminate("No listeners");else if(this._waitingFor===
WAITING_FOR_DEPENDS_TASKS)this._checkIfDependsTaskDone(status);else if(status.isIdle&&this._waitingFor===WAITING_FOR_WORKER)this._terminate()};PromiseTask.prototype._checkIfDependsTaskDone=function(status){var terminatedDependsTasks=0;if(status)terminatedDependsTasks=status.terminatedDependsTasks;if(terminatedDependsTasks!==this._dependsOnTasks.length)return;var that=this;this._waitingFor=WAITING_FOR_PROMISE;this._workerInputRetreiver["preWorkerProcess"](this._results,this._dependsOnTasks,this._taskKey).then(function(result){if(that._waitingFor!==
TERMINATED){that._waitingFor=WAITING_FOR_WORKER;that._callbacks["onDataReadyToProcess"](result)}})["catch"](function(reason){that._terminate(reason)})};PromiseTask.prototype._terminate=function(reason){if(this._waitingFor!==TERMINATED){this._waitingFor=TERMINATED;this._callbacks["onTerminated"](reason)}};asyncProxyScriptBlob.addMember(PromiseTaskClosure,"PromiseTask");return PromiseTask}var PromiseTask=PromiseTaskClosure();function PromiseDependencyWorkersClosure(){var asyncProxyScriptBlob=self["asyncProxyScriptBlob"];function PromiseDependencyWorkers(scriptsToImport,ctorName,ctorArgs,promiseInputRetreiver){var inputRetreiver=createInputRetreiverWrapper(promiseInputRetreiver);DependencyWorkers.call(this,scriptsToImport,ctorName,ctorArgs,inputRetreiver);if(!promiseInputRetreiver["getDependsOnTasks"])throw"AsyncProxy.DependencyWorkers: No "+"promiseInputRetreiver.getDependsOnTasks() method";if(!promiseInputRetreiver["preWorkerProcess"])throw"AsyncProxy.DependencyWorkers: No "+
...
ScriptsToImportPool.prototype["getScriptsForWorkerImport"]=ScriptsToImportPool.prototype.getScriptsForWorkerImport;DependencyWorkers.prototype["startTask"]=DependencyWorkers.prototype.startTask;DependencyWorkersTaskHandle.prototype["hasData"]=DependencyWorkersTaskHandle.prototype.hasData;DependencyWorkersTaskHandle.prototype["getLastData"]=DependencyWorkersTaskHandle.prototype.getLastData;DependencyWorkersTaskHandle.prototype["setPriority"]=DependencyWorkersTaskHandle.prototype.setPriority;DependencyWorkersTaskHandle.prototype["unregister"]=
DependencyWorkersTaskHandle.prototype.unregister;PromiseTask.prototype["onDependencyTaskResult"]=PromiseTask.prototype.onDependencyTaskResult;PromiseTask.prototype["statusUpdated"]=PromiseTask.prototype.statusUpdated;PromiseDependencyWorkers["createInputRetreiverWrapper"]=PromiseDependencyWorkers.prototype.createInputRetreiverWrapper}asyncProxyScriptBlob.addMember(ExportAsyncProxySymbolsClosure,"ExportAsyncProxySymbols");asyncProxyScriptBlob.addStatement("ExportAsyncProxySymbols("+"SubWorkerEmulationForChrome, AsyncProxySlaveSingleton, AsyncProxyMaster, ScriptsToImportPool, "+
"DependencyWorkers, DependencyWorkersTaskHandle, PromiseTask, PromiseDependencyWorkers);");asyncProxyScriptBlob.addStatement("self['AsyncProxy']['AsyncProxySlaveSingleton'] = AsyncProxySlaveSingleton;");asyncProxyScriptBlob.addStatement("self['AsyncProxy']['AsyncProxyMaster'] = AsyncProxyMaster;");asyncProxyScriptBlob.addStatement("self['AsyncProxy']['ScriptsToImportPool'] = ScriptsToImportPool;");asyncProxyScriptBlob.addStatement("self['AsyncProxy']['DependencyWorkers'] = DependencyWorkers;");asyncProxyScriptBlob.addStatement("self['AsyncProxy']['PromiseTask'] = PromiseTask;");
asyncProxyScriptBlob.addStatement("self['AsyncProxy']['PromiseDependencyWorkers'] = PromiseDependencyWorkers;");return ExportAsyncProxySymbols}ExportAsyncProxySymbolsClosure()(SubWorkerEmulationForChrome,AsyncProxySlaveSingleton,AsyncProxyMaster,ScriptsToImportPool,DependencyWorkers,DependencyWorkersTaskHandle,PromiseTask,PromiseDependencyWorkers);self["AsyncProxy"]["AsyncProxySlaveSingleton"]=AsyncProxySlaveSingleton;self["AsyncProxy"]["AsyncProxyMaster"]=AsyncProxyMaster;
self["AsyncProxy"]["ScriptsToImportPool"]=ScriptsToImportPool;self["AsyncProxy"]["DependencyWorkers"]=DependencyWorkers;self["AsyncProxy"]["PromiseTask"]=PromiseTask;self["AsyncProxy"]["PromiseDependencyWorkers"]=PromiseDependencyWorkers;
